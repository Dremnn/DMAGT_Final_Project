    ...
    : wxPanel(parent)
{
    ...
    m_nodes.push_back({ wxPoint(543, 360), true,_T("UBND Thành phố Hồ Chí Minh") }); // Node 31
	...
	//Update active nodes
    m_nodes.push_back({ wxPoint(432, 503), true,_T("Phố đi bộ Bùi Viện") }); // Node 57
    m_nodes.push_back({ wxPoint(610, 180), true,_T("Thảo Cầm Viên Sài Gòn") }); // Node 58
    m_nodes.push_back({ wxPoint(500, 105), true,_T("Chùa Ngọc Hoàng") }); // Node 59
    m_nodes.push_back({ wxPoint(406, 303), true,_T("Bảo Tàng Chứng tích Chiến Tranh") }); // Node 60
    m_nodes.push_back({ wxPoint(436, 513), false, wxEmptyString }); // Node 61
    m_nodes.push_back({ wxPoint(506, 110), false, wxEmptyString }); // Node 62
    m_nodes.push_back({ wxPoint(489, 129), false, wxEmptyString }); // Node 63
    m_nodes.push_back({ wxPoint(462, 105), false, wxEmptyString }); // Node 64
    m_nodes.push_back({ wxPoint(386, 184), false, wxEmptyString }); // Node 65
    m_nodes.push_back({ wxPoint(354, 217), false, wxEmptyString }); // Node 66
    m_nodes.push_back({ wxPoint(457, 250), false, wxEmptyString }); // Node 67
    m_nodes.push_back({ wxPoint(456, 252), false, wxEmptyString }); // Node 68
    m_nodes.push_back({ wxPoint(456, 255), false, wxEmptyString }); // Node 69
    m_nodes.push_back({ wxPoint(457, 258), false, wxEmptyString }); // Node 70
    m_nodes.push_back({ wxPoint(459, 260), false, wxEmptyString }); // Node 71
    m_nodes.push_back({ wxPoint(462, 261), false, wxEmptyString }); // Node 72
    m_nodes.push_back({ wxPoint(465, 260), false, wxEmptyString }); // Node 73
    m_nodes.push_back({ wxPoint(467, 259), false, wxEmptyString }); // Node 74
    m_nodes.push_back({ wxPoint(515, 101), false, wxEmptyString }); // Node 75
    m_nodes.push_back({ wxPoint(523, 100), false, wxEmptyString }); // Node 76
    m_nodes.push_back({ wxPoint(577, 213), false, wxEmptyString }); // Node 77
    m_nodes.push_back({ wxPoint(363, 259), false, wxEmptyString }); // Node 78
    m_nodes.push_back({ wxPoint(337, 233), false, wxEmptyString }); // Node 79
    m_nodes.push_back({ wxPoint(415, 309), false, wxEmptyString }); // Node 80
    m_nodes.push_back({ wxPoint(378, 348), false, wxEmptyString }); // Node 81
    m_nodes.push_back({ wxPoint(338, 393), false, wxEmptyString }); // Node 82
    m_nodes.push_back({ wxPoint(416, 435), false, wxEmptyString }); // Node 83
    m_nodes.push_back({ wxPoint(453, 495), false, wxEmptyString }); // Node 84
    m_nodes.push_back({ wxPoint(445, 275), false, wxEmptyString }); // Node 85
    m_nodes.push_back({ wxPoint(535, 370), false, wxEmptyString }); // Node 86
    m_nodes.push_back({ wxPoint(6, 0), false, wxEmptyString }); // Node 87
    m_nodes.push_back({ wxPoint(624, 492), true,_T("Bến Nhà Rồng") }); // Node 88
    m_nodes.push_back({ wxPoint(5, 0), false, wxEmptyString }); // Node 89
    m_nodes.push_back({ wxPoint(4, 0), false, wxEmptyString }); // Node 90
    m_nodes.push_back({ wxPoint(3, 0), false, wxEmptyString }); // Node 91
    m_nodes.push_back({ wxPoint(2, 0), false, wxEmptyString }); // Node 92
    m_nodes.push_back({ wxPoint(1, 0), false, wxEmptyString }); // Node 93
    m_nodes.push_back({ wxPoint(0, 0), false, wxEmptyString }); // Node 94
    m_nodes.push_back({ wxPoint(627, 408), true,_T("Hotel Majestic Saigon") }); // Node 95
    m_nodes.push_back({ wxPoint(592, 502), false, wxEmptyString }); // Node 96
    m_nodes.push_back({ wxPoint(567, 444), false, wxEmptyString }); // Node 97
    m_nodes.push_back({ wxPoint(586, 400), false, wxEmptyString }); // Node 98
    m_nodes.push_back({ wxPoint(626, 436), false, wxEmptyString }); // Node 99
    m_nodes.push_back({ wxPoint(548, 445), false, wxEmptyString }); // Node 100
    m_nodes.push_back({ wxPoint(625, 448), false, wxEmptyString }); // Node 101
...
void MapPanel::CreateGraph() {
    ...
    m_adajacentList[57][61] = calculateDistance(m_nodes[57].pos, m_nodes[61].pos);
    m_adajacentList[61][47] = calculateDistance(m_nodes[61].pos, m_nodes[47].pos);
    m_adajacentList[47][48] = calculateDistance(m_nodes[47].pos, m_nodes[48].pos);
    m_adajacentList[47][3] = calculateDistance(m_nodes[47].pos, m_nodes[3].pos);
    m_adajacentList[48][49] = calculateDistance(m_nodes[48].pos, m_nodes[49].pos);
    m_adajacentList[49][32] = calculateDistance(m_nodes[49].pos, m_nodes[32].pos);
    m_adajacentList[58][6] = calculateDistance(m_nodes[58].pos, m_nodes[6].pos);
    m_adajacentList[32][39] = calculateDistance(m_nodes[32].pos, m_nodes[39].pos);
    m_adajacentList[6][39] = calculateDistance(m_nodes[6].pos, m_nodes[39].pos);
    m_adajacentList[39][33] = calculateDistance(m_nodes[39].pos, m_nodes[33].pos);
    m_adajacentList[33][34] = calculateDistance(m_nodes[33].pos, m_nodes[34].pos);
    m_adajacentList[34][0] = calculateDistance(m_nodes[34].pos, m_nodes[0].pos);
    m_adajacentList[0][39] = calculateDistance(m_nodes[0].pos, m_nodes[39].pos);
    m_adajacentList[39][6] = calculateDistance(m_nodes[39].pos, m_nodes[6].pos);
    m_adajacentList[6][35] = calculateDistance(m_nodes[6].pos, m_nodes[35].pos);
    m_adajacentList[35][77] = calculateDistance(m_nodes[35].pos, m_nodes[77].pos);
    m_adajacentList[77][58] = calculateDistance(m_nodes[77].pos, m_nodes[58].pos);
    m_adajacentList[77][63] = calculateDistance(m_nodes[77].pos, m_nodes[63].pos);
    m_adajacentList[63][62] = calculateDistance(m_nodes[63].pos, m_nodes[62].pos);
    m_adajacentList[62][59] = calculateDistance(m_nodes[62].pos, m_nodes[59].pos);
    m_adajacentList[3][47] = calculateDistance(m_nodes[3].pos, m_nodes[47].pos);
    m_adajacentList[47][61] = calculateDistance(m_nodes[47].pos, m_nodes[61].pos);
    m_adajacentList[61][57] = calculateDistance(m_nodes[61].pos, m_nodes[57].pos);
    m_adajacentList[59][62] = calculateDistance(m_nodes[59].pos, m_nodes[62].pos);
    m_adajacentList[62][63] = calculateDistance(m_nodes[62].pos, m_nodes[63].pos);
    m_adajacentList[63][64] = calculateDistance(m_nodes[63].pos, m_nodes[64].pos);
    m_adajacentList[64][65] = calculateDistance(m_nodes[64].pos, m_nodes[65].pos);
    m_adajacentList[65][66] = calculateDistance(m_nodes[65].pos, m_nodes[66].pos);
    m_adajacentList[65][67] = calculateDistance(m_nodes[65].pos, m_nodes[67].pos);
    m_adajacentList[67][68] = calculateDistance(m_nodes[67].pos, m_nodes[68].pos);
    m_adajacentList[68][69] = calculateDistance(m_nodes[68].pos, m_nodes[69].pos);
    m_adajacentList[69][70] = calculateDistance(m_nodes[69].pos, m_nodes[70].pos);
    m_adajacentList[70][71] = calculateDistance(m_nodes[70].pos, m_nodes[71].pos);
    m_adajacentList[71][72] = calculateDistance(m_nodes[71].pos, m_nodes[72].pos);
    m_adajacentList[72][73] = calculateDistance(m_nodes[72].pos, m_nodes[73].pos);
    m_adajacentList[73][74] = calculateDistance(m_nodes[73].pos, m_nodes[74].pos);
    m_adajacentList[74][37] = calculateDistance(m_nodes[74].pos, m_nodes[37].pos);
    m_adajacentList[37][33] = calculateDistance(m_nodes[37].pos, m_nodes[33].pos);
    m_adajacentList[33][34] = calculateDistance(m_nodes[33].pos, m_nodes[34].pos);
    m_adajacentList[34][0] = calculateDistance(m_nodes[34].pos, m_nodes[0].pos);
    m_adajacentList[66][0] = calculateDistance(m_nodes[66].pos, m_nodes[0].pos);
    m_adajacentList[59][62] = calculateDistance(m_nodes[59].pos, m_nodes[62].pos);
    m_adajacentList[62][75] = calculateDistance(m_nodes[62].pos, m_nodes[75].pos);
    m_adajacentList[75][76] = calculateDistance(m_nodes[75].pos, m_nodes[76].pos);
    m_adajacentList[76][58] = calculateDistance(m_nodes[76].pos, m_nodes[58].pos);
    m_adajacentList[60][78] = calculateDistance(m_nodes[60].pos, m_nodes[78].pos);
    m_adajacentList[78][63] = calculateDistance(m_nodes[78].pos, m_nodes[63].pos);
    m_adajacentList[63][62] = calculateDistance(m_nodes[63].pos, m_nodes[62].pos);
    m_adajacentList[62][59] = calculateDistance(m_nodes[62].pos, m_nodes[59].pos);
    m_adajacentList[59][62] = calculateDistance(m_nodes[59].pos, m_nodes[62].pos);
    m_adajacentList[62][63] = calculateDistance(m_nodes[62].pos, m_nodes[63].pos);
    ...
    m_adajacentList[64][65] = calculateDistance(m_nodes[64].pos, m_nodes[65].pos);
    m_adajacentList[65][66] = calculateDistance(m_nodes[65].pos, m_nodes[66].pos);
    m_adajacentList[66][79] = calculateDistance(m_nodes[66].pos, m_nodes[79].pos);
    m_adajacentList[79][60] = calculateDistance(m_nodes[79].pos, m_nodes[60].pos);
    m_adajacentList[60][80] = calculateDistance(m_nodes[60].pos, m_nodes[60].pos);
    m_adajacentList[80][81] = calculateDistance(m_nodes[80].pos, m_nodes[81].pos);
    m_adajacentList[81][45] = calculateDistance(m_nodes[81].pos, m_nodes[45].pos);
    m_adajacentList[60][80] = calculateDistance(m_nodes[60].pos, m_nodes[80].pos);
    m_adajacentList[80][81] = calculateDistance(m_nodes[80].pos, m_nodes[81].pos);
    m_adajacentList[81][82] = calculateDistance(m_nodes[81].pos, m_nodes[82].pos);
    m_adajacentList[82][83] = calculateDistance(m_nodes[82].pos, m_nodes[83].pos);
    m_adajacentList[83][84] = calculateDistance(m_nodes[83].pos, m_nodes[84].pos);
    m_adajacentList[84][61] = calculateDistance(m_nodes[84].pos, m_nodes[61].pos);
    m_adajacentList[0][39] = calculateDistance(m_nodes[0].pos, m_nodes[39].pos);
    m_adajacentList[39][33] = calculateDistance(m_nodes[39].pos, m_nodes[33].pos);
    m_adajacentList[33][85] = calculateDistance(m_nodes[33].pos, m_nodes[85].pos);
    m_adajacentList[85][80] = calculateDistance(m_nodes[85].pos, m_nodes[80].pos);
    m_adajacentList[80][60] = calculateDistance(m_nodes[80].pos, m_nodes[60].pos);
    m_adajacentList[31][86] = calculateDistance(m_nodes[31].pos, m_nodes[86].pos);
    m_adajacentList[86][49] = calculateDistance(m_nodes[86].pos, m_nodes[49].pos);
    m_adajacentList[31][5] = calculateDistance(m_nodes[31].pos, m_nodes[5].pos);
    m_adajacentList[48][86] = calculateDistance(m_nodes[48].pos, m_nodes[86].pos);
    m_adajacentList[86][31] = calculateDistance(m_nodes[86].pos, m_nodes[31].pos);
    m_adajacentList[5][31] = calculateDistance(m_nodes[5].pos, m_nodes[31].pos);
    m_adajacentList[88][96] = calculateDistance(m_nodes[88].pos, m_nodes[96].pos);
    m_adajacentList[96][26] = calculateDistance(m_nodes[96].pos, m_nodes[26].pos);
    m_adajacentList[26][86] = calculateDistance(m_nodes[26].pos, m_nodes[86].pos);
    m_adajacentList[95][88] = calculateDistance(m_nodes[95].pos, m_nodes[88].pos);
    m_adajacentList[88][99] = calculateDistance(m_nodes[88].pos, m_nodes[99].pos);
    m_adajacentList[99][24] = calculateDistance(m_nodes[99].pos, m_nodes[24].pos);
    m_adajacentList[88][95] = calculateDistance(m_nodes[88].pos, m_nodes[95].pos);
    m_adajacentList[96][97] = calculateDistance(m_nodes[96].pos, m_nodes[96].pos);
    m_adajacentList[97][47] = calculateDistance(m_nodes[97].pos, m_nodes[47].pos);
    m_adajacentList[97][55] = calculateDistance(m_nodes[97].pos, m_nodes[55].pos);
    m_adajacentList[25][100] = calculateDistance(m_nodes[25].pos, m_nodes[100].pos);
    m_adajacentList[100][101] = calculateDistance(m_nodes[100].pos, m_nodes[101].pos);
    m_adajacentList[101][88] = calculateDistance(m_nodes[101].pos, m_nodes[88].pos);
    m_adajacentList[95][101] = calculateDistance(m_nodes[95].pos, m_nodes[101].pos);
    m_adajacentList[101][55] = calculateDistance(m_nodes[101].pos, m_nodes[55].pos);
    m_adajacentList[55][97] = calculateDistance(m_nodes[55].pos, m_nodes[97].pos);
    m_adajacentList[97][26] = calculateDistance(m_nodes[97].pos, m_nodes[26].pos);
    m_adajacentList[26][48] = calculateDistance(m_nodes[26].pos, m_nodes[48].pos);
    m_adajacentList[95][99] = calculateDistance(m_nodes[95].pos, m_nodes[99].pos);
    m_adajacentList[99][24] = calculateDistance(m_nodes[99].pos, m_nodes[24].pos);
    m_adajacentList[24][4] = calculateDistance(m_nodes[24].pos, m_nodes[4].pos);
    m_adajacentList[4][24] = calculateDistance(m_nodes[4].pos, m_nodes[24].pos);
    m_adajacentList[24][99] = calculateDistance(m_nodes[24].pos, m_nodes[99].pos);
    m_adajacentList[99][95] = calculateDistance(m_nodes[99].pos, m_nodes[95].pos);
    m_adajacentList[24][5] = calculateDistance(m_nodes[24].pos, m_nodes[5].pos);
    m_adajacentList[4][24] = calculateDistance(m_nodes[4].pos, m_nodes[24].pos);
    m_adajacentList[24][99] = calculateDistance(m_nodes[24].pos, m_nodes[99].pos);
    m_adajacentList[99][88] = calculateDistance(m_nodes[99].pos, m_nodes[88].pos);
